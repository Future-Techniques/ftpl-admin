...
export function IndustriesPage() {
  ...
  const fetchIndustries = async () => {
    try {
      ...
      const res = await api.get('/admin/industries', { params });
      if (res.data.result === 'success') {
        setIndustries(res.data.industries);
      }
    } catch (err) {
      showToast('error', 'Failed to load industry sectors');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEditModal = (industry) => {
    ...
    setImageFile(null);
    setImagePreview(industry.image || null);
    setIsModalOpen(true);
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!formData.title.trim()) {
      showToast('error', 'Industry sector title is required');
      return;
    }
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        if (res.data.result === 'success') {
          showToast('success', 'Industry sector created successfully');
          setIsModalOpen(false);
          fetchIndustries();
        }
      } else {
        const res = await api.put(`/admin/industries/${currentIndustry.id}`, data, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        if (res.data.result === 'success') {
          showToast('success', 'Industry sector updated successfully');
          setIsModalOpen(false);
          fetchIndustries();
        }
      }
  ...
  const handleToggleStatus = async (ind) => {
    try {
      const res = await api.patch(`/admin/industries/${ind.id}/status`);
      if (res.data.result === 'success') {
        setIndustries((prev) =>
          prev.map((item) => (item.id === ind.id ? { ...item, status: res.data.status } : item))
        );
        showToast('success', `Sector ${res.data.status === 1 ? 'activated' : 'deactivated'}`);
      }
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeleting(true);
      const res = await api.delete(`/admin/industries/${industryToDelete.id}`);
      if (res.data.result === 'success') {
        showToast('success', 'Industry sector deleted successfully');
        setDeleteModalOpen(false);
        setIndustryToDelete(null);
        fetchIndustries();
      }
    } catch (err) {
      showToast('error', err.response?.data?.message || 'Failed to delete sector');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#382E42] tracking-tight flex items-center gap-2.5">
            <Factory className="w-7 h-7 text-[#574B66]" />
            <span>Target Industries & Sectors</span>
          </h1>
          <p className="text-sm text-slate-600 mt-1">
            Manage industry verticals (Distillery, Cement, Textiles, Pharmaceuticals, Food & Beverage).
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-700">
            <thead className="bg-[#F8F6FA] text-[11px] uppercase tracking-wider text-slate-600 font-bold border-b border-[#E8E2EE]">
              <tr>
                <th className="px-5 py-3.5">Industry Sector</th>
                <th className="px-4 py-3.5">Sector Overview</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan="5" className="px-6 py-16 text-center text-slate-400">
                    ...
                    <p className="text-sm font-medium">Fetching industries ...</p>
                  </td>
                </tr>
              ) : industries.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-slate-400">
                    <Factory className="w-10 h-10 mx-auto text-slate-700 mb-2" />
                    <p className="text-base font-semibold text-slate-600">No industries found</p>
                  </td>
                </tr>
              ) : (
                industries.map((ind) => (
                  <tr key={ind.id} className="hover:bg-brand-50/40 transition group">
                    ...
                    <td className="px-4 py-4 max-w-xs text-xs text-slate-600 truncate">
                      ...
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${
                            ind.status === 1 ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'
                          }`}
                        />
                        <span>{ind.status === 1 ? 'Active' : 'Inactive'}</span>
                      </button>
                    </td>
          ...
          <div className="relative w-full max-w-xl bg-white border border-[#E8E2EE] rounded-2xl shadow-2xl p-6 md:p-8 my-8">
            <div className="flex items-center justify-between pb-4 border-b border-[#E8E2EE] mb-6">
              <h2 className="text-xl font-extrabold text-[#382E42]">
                {modalMode === 'create' ? 'Add Industry Vertical' : 'Edit Industry Vertical'}
              </h2>
              <button
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Distilleries & Breweries"
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. distilleries-and-breweries"
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#574B66] mb-1.5">
                  Sector Description / Applications
                </label>
                <textarea
                  rows="3"
                  value={formData.description}
                  onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                  placeholder="Overview of industrial thermal requirements in this sector..."
                  className="w-full px-3.5 py-2.5 bg-white border border-[#E8E2EE] rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#574B66]/20 focus:border-[#574B66]"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#E8E2EE]">
                ...
                >
                  {submitting ? (
                    ...
                    <span>{modalMode === 'create' ? 'Create Sector' : 'Save Changes'}</span>
                  )}
                </button>
              </div>
            </form>
      ...
      {deleteModalOpen && industryToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white border border-[#E8E2EE] rounded-2xl shadow-2xl p-6 space-y-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-[#382E42]">Delete Industry Vertical?</h3>
              <p className="text-sm text-slate-600 mt-1">
                Are you sure you want to delete <span className="text-[#574B66] font-bold">"{industryToDelete.title}"</span>?
