...
export function FeaturesPage() {
  ...
  const fetchFeatures = async () => {
    try {
      ...
      const res = await api.get('/admin/features', { params });
      if (res.data.result === 'success') {
        setFeatures(res.data.features);
      }
    } catch (err) {
      showToast('error', 'Failed to load core features');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!formData.title.trim()) {
      showToast('error', 'Feature title is required');
      return;
    }
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        if (res.data.result === 'success') {
          showToast('success', 'Core feature created successfully');
          setIsModalOpen(false);
          fetchFeatures();
        }
      } else {
        const res = await api.put(`/admin/features/${currentFeature.id}`, data, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        if (res.data.result === 'success') {
          showToast('success', 'Core feature updated successfully');
          setIsModalOpen(false);
          fetchFeatures();
        }
      }
  ...
  const handleToggleStatus = async (f) => {
    try {
      const res = await api.patch(`/admin/features/${f.id}/status`);
      if (res.data.result === 'success') {
        setFeatures((prev) =>
          prev.map((item) => (item.id === f.id ? { ...item, status: res.data.status } : item))
        );
        showToast('success', `Feature ${res.data.status === 1 ? 'activated' : 'deactivated'}`);
      }
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeleting(true);
      const res = await api.delete(`/admin/features/${featureToDelete.id}`);
      if (res.data.result === 'success') {
        showToast('success', 'Core feature deleted successfully');
        setDeleteModalOpen(false);
        setFeatureToDelete(null);
        fetchFeatures();
      }
    } catch (err) {
      showToast('error', err.response?.data?.message || 'Failed to delete feature');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#382E42] tracking-tight flex items-center gap-2.5">
            <Boxes className="w-7 h-7 text-[#574B66]" />
            <span>Core Engineering Features</span>
          </h1>
          <p className="text-sm text-slate-600 mt-1">
            Manage key value propositions (Engineering Support, Steam & Energy Audits, Water Quality Analysis).
          </p>
        </div>
      ...
      <div className="p-4 rounded-2xl bg-white border border-[#E8E2EE] flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && fetchFeatures()}
            placeholder="Search features by title or scope..."
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-700">
            <thead className="bg-[#F8F6FA] text-[11px] uppercase tracking-wider text-slate-600 font-bold border-b border-[#E8E2EE]">
              <tr>
                <th className="px-5 py-3.5">Feature & Proposition</th>
                <th className="px-4 py-3.5">Linked Target</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan="5" className="px-6 py-16 text-center text-slate-400">
                    ...
                    <p className="text-sm font-medium">Fetching core features ...</p>
                  </td>
                </tr>
              ) : features.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-slate-400">
                    <Boxes className="w-10 h-10 mx-auto text-slate-700 mb-2" />
                    <p className="text-base font-semibold text-slate-600">No features found</p>
                  </td>
                </tr>
              ) : (
                features.map((f) => (
                  <tr key={f.id} className="hover:bg-brand-50/40 transition group">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3.5">
                        <div className="w-14 h-14 rounded-xl bg-white border border-[#E8E2EE] flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
                          {f.image ? (
                            <img
                              src={f.image}
                              alt={f.title}
                        ...
                        <div className="min-w-0 max-w-sm">
                          ...
                          <p className="text-xs text-slate-600 truncate mt-0.5 line-clamp-1">
                            {f.description ? f.description.replace(/<[^>]*>?/gm, '') : 'No description'}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      {f.link ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-surface-base text-slate-700 text-xs font-mono">
                          <Link2 className="w-3 h-3 text-[#574B66]" />
                          <span>{f.link}</span>
                        </span>
                      ) : (
          ...
          <div className="relative w-full max-w-xl bg-white border border-[#E8E2EE] rounded-2xl shadow-2xl p-6 md:p-8 my-8">
            <div className="flex items-center justify-between pb-4 border-b border-[#E8E2EE] mb-6">
              <h2 className="text-xl font-extrabold text-[#382E42]">
                {modalMode === 'create' ? 'Add Core Feature' : 'Edit Core Feature'}
              </h2>
              <button
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Turnkey Installation & Engineering Support"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="/services/steam-audit or #inquire"
                    className="w-full px-3.5 py-2.5 bg-white border border-[#E8E2EE] rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#574B66]/20 focus:border-[#574B66]"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#574B66] mb-1.5">
                  Feature Graphic / Illustration (Media Storage)
                </label>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Short compelling description of this technical value proposition..."
                  className="w-full px-3.5 py-2.5 bg-white border border-[#E8E2EE] rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#574B66]/20 focus:border-[#574B66]"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                ...
                <div className="flex items-center pt-6">
                  <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-slate-700">
                    ...
                    />
                    <span>Active Feature Status</span>
                  </label>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#E8E2EE]">
                ...
                >
                  {submitting ? (
                    ...
                    <span>{modalMode === 'create' ? 'Create Feature' : 'Save Changes'}</span>
                  )}
                </button>
              </div>
            </form>
      ...
      {deleteModalOpen && featureToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white border border-[#E8E2EE] rounded-2xl shadow-2xl p-6 space-y-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-[#382E42]">Delete Core Feature?</h3>
              <p className="text-sm text-slate-600 mt-1">
                Are you sure you want to delete <span className="text-[#574B66] font-bold">"{featureToDelete.title}"</span>?
                This action is permanent and removes the feature graphic  Storage.
              </p>
            </div>
