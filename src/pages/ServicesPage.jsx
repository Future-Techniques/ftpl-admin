...
export function ServicesPage() {
  ...
  const fetchServices = async () => {
    try {
      ...
      showToast('error', 'Failed to load engineering services');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        const res = await api.put(`/admin/services/${currentService.id}`, data, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        if (res.data.result === 'success') {
  ...
  const handleToggleStatus = async (service) => {
    try {
      const res = await api.patch(`/admin/services/${service.id}/status`);
      if (res.data.result === 'success') {
        setServices((prev) =>
          prev.map((s) => (s.id === service.id ? { ...s, status: res.data.status } : s))
        );
        showToast('success', `Service ${res.data.status === 1 ? 'activated' : 'deactivated'}`);
      }
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#382E42] tracking-tight flex items-center gap-2.5">
            <Wrench className="w-7 h-7 text-[#574B66]" />
            <span>Industrial Engineering Services</span>
          </h1>
          <p className="text-sm text-slate-600 mt-1">
            Manage comprehensive turnkey solutions, maintenance contracts, and technical consultation.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-700">
            <thead className="bg-[#F8F6FA] text-[11px] uppercase tracking-wider text-slate-600 font-bold border-b border-[#E8E2EE]">
              <tr>
                <th className="px-5 py-3.5">Engineering Service</th>
                <th className="px-4 py-3.5">Summary / Scope</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-slate-400">
                    <Wrench className="w-10 h-10 mx-auto text-slate-700 mb-2" />
                      ...
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${
                            s.status === 1 ? 'bg-emerald-400 animate-pulse' : 'bg-slate-500'
                          }`}
                        />
                        <span>{s.status === 1 ? 'Active' : 'Inactive'}</span>
                      </button>
                    </td>
          ...
          <div className="relative w-full max-w-2xl bg-white border border-[#E8E2EE] rounded-2xl shadow-2xl p-6 md:p-8 my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 border-b border-[#E8E2EE] mb-6">
              <h2 className="text-xl font-extrabold text-[#382E42]">
                {modalMode === 'create' ? 'Create New Engineering Service' : 'Edit Engineering Service'}
              </h2>
              <button
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Turnkey Boiler Installation"
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#574B66] mb-1.5">
                  Service Cover Image (Media Storage)
                </label>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. 24/7 emergency response, IBR certified testing, OEM genuine parts"
                  className="w-full px-3.5 py-2.5 bg-white border border-[#E8E2EE] rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#574B66]/20 focus:border-[#574B66]"
                />
              </div>
      ...
      {deleteModalOpen && serviceToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white border border-[#E8E2EE] rounded-2xl shadow-2xl p-6 space-y-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-[#382E42]">Delete Engineering Service?</h3>
              <p className="text-sm text-slate-600 mt-1">
                Are you sure you want to delete <span className="text-[#574B66] font-bold">"{serviceToDelete.title}"</span>?
                This action is permanent and removes the service listing .
              </p>
            </div>
